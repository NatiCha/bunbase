...
TSBase provides built-in authentication via HTTP endpoints. Auth uses session cookies — no token management required on the client.
...
Sets `tsbase_session` and `csrf_token` cookies. Fields like `password_hash` are never returned.
...
Always returns success to prevent user enumeration. In development mode, the reset token is logged to the console. In production, TSBase POSTs to your configured `auth.email.webhook`:
...
TSBase supports Google, GitHub, and Discord OAuth. Configure providers in your config:
...
2. TSBase redirects to the provider's consent screen
3. After consent, the provider redirects to `/auth/oauth/<provider>/callback`
4. TSBase creates or links the user account, sets session cookies, and redirects to `redirectUrl`
...
- Session cookie name: `tsbase_session`
