...
A TypeScript-native backend-as-a-service built on Bun, Drizzle ORM, and tRPC. Define your schema, set access rules, and get a full API — auth, CRUD, and file storage — with zero boilerplate.
...
- **Type-safe client SDK** — tRPC-powered frontend client with auth and file helpers
...
8. [Configuration](/configuration/) — full `defineConfig` reference
9. [Extending](/extending/) — add custom tRPC routes
10. [Deployment](/deployment/) — production checklist
...
That's it. You now have a full REST API with auth, CRUD, and file storage.
