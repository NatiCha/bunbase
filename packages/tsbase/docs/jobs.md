...
Jobs start automatically when `tsbase.listen()` is called, after the database has finished bootstrapping.
...
```
[TSBase] Job "daily-digest" failed: Error: SMTP connection refused
```
...
```
[TSBase] Job "cleanup-sessions" is still running from previous tick — skipping
```
