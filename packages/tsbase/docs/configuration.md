...
Use `defineConfig` to customize TSBase behavior. All options are optional — sensible defaults are provided.

## Full interface

```ts
import { defineConfig } from "tsbase";
...
When `true`, TSBase runs in development mode:
...
Directory where Drizzle Kit stores generated migration files. TSBase runs all pending migrations on startup.
...
A URL that TSBase will POST to when a password reset is requested. The webhook receives:
