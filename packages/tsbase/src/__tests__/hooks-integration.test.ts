...
const root = join(tmpdir(), `tsbase-hooks-intg-${Date.now()}`);
...
let tsbaseModify: ReturnType<typeof createServer>;
...
let tsbaseBlock: ReturnType<typeof createServer>;
...
beforeAll(async () => {
  ...
  await tsbaseBlock.adapter.rawExecute(
    "CREATE TABLE IF NOT EXISTS items (id TEXT PRIMARY KEY, name TEXT NOT NULL, tag TEXT)",
  );
  await tsbaseBlock.adapter.rawExecute(
  ...
  serverBlock = tsbaseBlock.listen(0);
  baseBlock = serverBlock.url.toString().replace(/\/$/, "");
});

afterAll(() => {
  serverBlock?.stop();
  tsbaseBlock?.adapter.close();
});
