...
function StatusBadge({ status }: { status: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded px-1.5 py-0.5 text-xs font-mono font-medium ${cls}`}
    >
      {status}
    </span>
  );
...
function MethodBadge({ method }: { method: string }) {
  ...
  return (
    <span
      className={`font-mono text-xs font-semibold ${colors[method] ?? "text-gray-600 dark:text-gray-400"}`}
    >
      {method}
    </span>
  );
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 bg-white dark:bg-gray-950">
            <tr className="border-b border-gray-100 dark:border-gray-800">
              <th className="w-20 px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">
                Method
              </th>
