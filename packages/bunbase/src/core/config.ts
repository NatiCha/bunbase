...
export interface BunBaseConfig {
  ...
  cookieDomain?: string;
  /**
   * Service key for server-to-server authentication.
   * Grants admin-level access without requiring a user account.
   * Falls back to `BUNBASE_SERVICE_KEY` env var.
   * If neither is provided, a key is auto-generated and persisted to `.bunbase-service-key`.
   */
  serviceKey?: string;
}
...
export interface ResolvedConfig {
  ...
  /** Domain for session and CSRF cookies. */
  cookieDomain?: string;
  /** Resolved service key (may be auto-generated by server). */
  serviceKey: string;
}
