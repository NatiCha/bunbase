...
const root = join(tmpdir(), `bunbase-rt-intg-${Date.now()}`);
...
beforeAll(async () => {
  ...
  await seedUser(bunbase.adapter, userAId, "usera@test.com", userASession);
  await seedUser(bunbase.adapter, userBId, "userb@test.com", userBSession);
});
...
test("realtime subscribe passes RuleArg defaults to list rule", async () => {
  ...
  const ws = new WebSocket(`${wsBase}/realtime`, {
    headers: { cookie: `bunbase_session=${userASession}` },
  });
  await waitForOpen(ws);
...
test("two auth clients subscribe to channel and receive broadcasts from each other", async () => {
  ...
  const wsB = new WebSocket(`${wsBase}/realtime`, {
    headers: { cookie: `bunbase_session=${userBSession}` },
  });
...
test("realtime disabled — /realtime returns 404", async () => {
  ...
  const disabledBunBase = createServer({
    schema: { tasks },
    config: makeResolvedConfig({
      development: true,
